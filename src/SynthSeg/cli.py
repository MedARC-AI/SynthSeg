...
RELEASE_URL = "https://github.com/MedARC-AI/SynthSeg/releases/download/v2.0"

labels_dir = str(pkg_resources.files("SynthSeg") / "data" / "labels_classes_priors")


def get_model_dir(cache_dir=None):
    if cache_dir is None:
        cache_dir = user_cache_dir("SynthSeg")
    model_dir = os.path.join(cache_dir, "SynthSeg_models")
    if not os.path.exists(model_dir):
        os.makedirs(cache_dir, exist_ok=True)
        url = f"{RELEASE_URL}/SynthSeg_models.zip"
        zip_path = os.path.join(cache_dir, "SynthSeg_models.zip")
        print("Downloading SynthSeg_models.zip ...")
